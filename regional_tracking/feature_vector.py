...
class FeatureVectorGenerator:
    ...
    def __init__(self, torch_device, cuda=False, model='resnet-18', layer='default', layer_output_size=512, gpu=0):
        """ Img2Vec
        ...
        self.torchTransforms = torchvision.transforms.Compose([
    ...
    def feature_vector_from_images(self, imgs, tensor=False):
        ...
        :param imgs: PIL Image or list of PIL Images
        :param tensor: If True, get_vec will return a FloatTensor instead of Numpy array
        :returns: Numpy ndarray
        """
        assert type(imgs) == list and len(imgs) > 0

        a = [self.torchTransforms(im) for im in imgs]
        images = torch.stack(a).to(self.device)
        if self.model_name in ['alexnet', 'vgg']:
            my_embedding = torch.zeros(len(imgs), self.layer_output_size)
            ...
            my_embedding = torch.zeros(len(imgs), self.layer_output_size, 1, 1)
    ...
    def feature_vector_from_image(self, img, tensor=False):
        """ Get vector embedding from PIL image
        :param img: PIL Image or list of PIL Images
        :param tensor: If True, get_vec will return a FloatTensor instead of Numpy array
        :returns: Numpy ndarray
        """
        image = self.torchTransforms(img).unsqueeze(0).to(self.device)
        if self.model_name in ['alexnet', 'vgg']:
